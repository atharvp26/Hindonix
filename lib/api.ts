...
export const getHeroDesktopImageFromRedis = (): Promise<{ url: string }> =>
  apiFetch<{ url: string }>("/hero-desktop-image");
...
export const getTestimonialsMobileBgFromRedis = (): Promise<string[]> =>
  apiFetch<string[]>("/testimonials-mobile-bg");

export const setTestimonialsMobileBgInRedis = (urls: string[]): Promise<string[]> =>
  apiFetch<string[]>("/testimonials-mobile-bg", { method: "PUT", body: JSON.stringify({ urls }) });
